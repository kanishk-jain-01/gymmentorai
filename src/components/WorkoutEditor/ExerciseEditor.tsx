...
const ExerciseEditor: React.FC<ExerciseEditorProps> = ({ exerciseIndex, control, register, errors, removeExercise }) => {
  ...
  return (
    <div 
      className="border border-subtle rounded-xl p-6 relative bg-theme-card hover:bg-theme-card/90 transition-all duration-200 group/exercise shadow-sm hover:shadow-md"
    >
      <div className="absolute -top-2 -right-2">
        <button
          type="button"
          onClick={() => removeExercise(exerciseIndex)}
          className="flex items-center justify-center h-7 w-7 rounded-full bg-red-100/90 text-error hover:bg-red-200 transition-all duration-200 opacity-0 group-hover/exercise:opacity-100 focus:opacity-100 shadow-sm transform hover:scale-110 active:scale-95"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
        <div className="md:col-span-2 group">
          <label className="block text-sm font-medium text-theme-fg/80 group-focus-within:text-primary transition-colors duration-200">
            Exercise Name
          </label>
          ...
          {errors.exercises?.[exerciseIndex]?.name && (
            <p className="mt-1.5 text-sm text-error animate-fadeIn flex items-center">
      ...
      <div className="pt-4 border-t border-theme-border/30">
        <div className="flex justify-between items-center mb-4">
          <h5 className="text-sm font-medium text-theme-fg text-gradient-primary">
            Sets
          </h5>
          <button
            type="button"
